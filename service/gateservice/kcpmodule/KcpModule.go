...
func (km *KcpModule) OnInit() error {
	//1解析配置
	iConfig := km.GetService().GetServiceCfg()
	if iConfig == nil {
		return fmt.Errorf("%s config is error", km.GetService().GetName())
	}

	mapKcpCfg := iConfig.(map[string]interface{})
	iKcpCfg, ok := mapKcpCfg["KcpCfg"]
	if ok == false {
		return fmt.Errorf("%s.TcpCfg config is error", km.GetService().GetName())
	}

	byteKcpCfg, err := json.Marshal(iKcpCfg)
	if err != nil {
		return fmt.Errorf("%s.TcpCfg config is error:%s", km.GetService().GetName(), err.Error())
	}
	...
	km.mapClient = make(map[string]*Client, kcpCfg.MaxConnNum)
	km.GetEventProcessor().RegEventReceiverFunc(event.Sys_Event_Kcp, km.GetEventHandler(), km.kcpEventHandler)
	km.process.SetByteOrder(kcpCfg.LittleEndian)
	km.kcpServer.Init(&kcpCfg)
...
func (km *KcpModule) SetProcessor(process processor.IRawProcessor) {
	km.process = process
}
