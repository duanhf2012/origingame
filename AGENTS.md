...
- AI 新增或重写业务函数时，按输入校验、读取或准备状态、执行核心业务、调用外部依赖、处理结果或清理资源等实际阶段组织代码；每个独立代码块前写一条简短中文注释；
- 注释只说明该步骤的目的或约束，例如“校验登录参数”“读取当前区服快照”“失败时回滚预占”，不逐行复述赋值、判断或循环；单一且显而易见的短语句不强制添加注释；
- 函数内语句应保持直白：使用完整业务名称，优先早返回，避免过深嵌套和难以理解的布尔组合；一个函数出现多个无关阶段或无法用简短注释概括时，应拆为职责明确的私有函数；
- AI 生成的注释、变量名和错误上下文必须与实际代码同步，避免模板化、过时或含糊的描述。
